...
void setup() {
  ...
  Serial.println("Word of the day");
}
...
void loop() {
  uint64_t sleepTime = 24 * HOUR;
  
  if (connectToWifi()) {
    String statusStr = getJSON(URL);
    json = JSON.parse(statusStr);
    Serial.print("json: "); Serial.println(json);

    if (JSON.typeof(json) == "undefined") {
      displayCenteredText("Error fetching JSON");
    } else {
      if (!fillWordsFromJson(json, &words)) {
        displayCenteredText("Error parsing JSON");
      } else {
        displayData(&words);
      }
    }
